//
//  BuShuControllerTableViewController.swift
//  wjdj
//
//  Created by HANBANG on 15/6/28.
...
import Alamofire
import SwiftyJSON
class BuShuController: UITableViewController {
    ...
    func SetTableData(page:Int,type:TABLETYPE,Sucess:(Tabledata:NSMutableArray)->Void)
    {
        ...
        //"djdt"
        self.pleaseWait()
        Alamofire.request(.GET, Constants().QUERYURL+"gznrjh",parameters: ["pn":page,"ps":"10"]).responseJSON { (request, response, string, error) in
    ...
    */
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier=="bushudetail"
        {
            var detalisController=segue.destinationViewController as! BuShuDetailController;
