...
class CirCleView: UIView, UIScrollViewDelegate {
    ...
    convenience init(frame: CGRect, imageArray: [UIImage!]?,titleArray:[String!]?) {
    ...
    private func setUpCircleView() {
        ...
        self.title=UILabel(frame: CGRectMake(5, self.frame.size.height - 20, 350, 20))
...
//        self.timer = NSTimer.scheduledTimerWithTimeInterval(TimeInterval, target: self, selector: "timerAction", userInfo: nil, repeats: true)
    ...
    private func setScrollViewOfImage(){
     ...
     {
        println(self.indexOfCurrentImage)
        println(self.imageArray.count)
          println(self.titleArray.count)
          println(self.titleArray[0])
        
        self.currentImageView.image = self.imageArray[self.indexOfCurrentImage]
        self.title.text=self.titleArray[self.indexOfCurrentImage]
