...
import Alamofire
import SwiftyJSON
class TuWenController: UIViewController, UITableViewDataSource,UITableViewDelegate {
    ...
    */
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier=="tuwenidex"
        {
            var detalisController:TuWenDetailController?
            var bVc:UINavigationController = segue.destinationViewController as! UINavigationController
            detalisController=bVc.viewControllers.first as? TuWenDetailController
